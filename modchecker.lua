...
-- Fungsi makeRequest sesuai signature GentaHax
function requestMods()
    ...
    if not res or not res.content then
        logToConsole("[X] Gagal ambil data dari API.")
        return nil
    end
...
function checkMods(jsonString)
    ...
    if not status then
        logToConsole("[X] Gagal decode JSON: " .. tostring(data))
        return
    end

    if not data.mods or type(data.mods) ~= "table" then
        logToConsole("[X] Data 'mods' tidak ditemukan di JSON.")
        return
    end
    ...
    elseif not kailyxLog then
        logToConsole("`2[✓] No MODs online. Safe farming~")
    end
end
